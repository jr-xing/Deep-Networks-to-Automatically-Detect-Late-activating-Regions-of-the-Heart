...
def TOS3DPlotInterp(dataOfPatient, toshow = 'TOSInterploated', vmin = None, vmax = None, interpolate = True, interpMethod = 'quadratic', azim=None, addSectors = False):
    patientID2Show = 'SET02\\EC03'
    # interpolate = True
    ...
    for sliceIdx, sliceData in enumerate(dataOfPatient):            
        ...
        if toshow in sliceData.keys():
            TOSMatOri[sliceIdx, :] = sliceData[toshow][0, sliceData['AnalysisFv'].layerid == 3]
            hasTOS = True
        else:
            hasTOS = False
    ...
    for ringLoc in range(xsMatOri.shape[1]):
        ...
        interpFuncX = interp1d(sliceSpatialLocsOri, xsColOri, kind=interpMethod)
        interpFuncY = interp1d(sliceSpatialLocsOri, ysColOri, kind=interpMethod)
        interpFuncTOS = interp1d(sliceSpatialLocsOri, TOSColOri, kind=interpMethod)
    ...
    if interpolate:
        ...
        color = color# / 17
        scatterPlot = axe.scatter(xsFlat, ysFlat, zsFlat, c = color, cmap='jet', zorder = 2, vmin = vmin, vmax = vmax)
    else:
        color = TOSOriFlat if hasTOS else zsOriFlat
        color = color# / 17
        scatterPlot = axe.scatter(xsOriFlat, ysOriFlat, zsOriFlat, c = color, cmap='jet', zorder = 2, vmin = vmin, vmax = vmax, azim = azim)
    axe.view_init(azim = azim)
    axe.set_xlabel('X')
    axe.set_ylabel('Y')
    ...
    plt.colorbar(scatterPlot, ax = axe, fraction=0.025, pad=0.04)
    ...
    # eye = plt.imread('./utils/eye_plot.gif')
