# -*- coding: utf-8 -*-
"""
Created on Sun Jan 31 14:06:55 2021
...
if loadExtractedData:
    dataFilename = 'D://dataFull-201-2020-12-23-Jerry.npy'
    dataSaved = np.load(dataFilename, allow_pickle = True).item()
    dataInfo = dataSaved['description']
    dataFull = dataSaved['data']
    
    # ↓ Data used for original submission. Replaced since the GT for CT11 is wrong
    ...
    # Add active contour result
    import scipy.io as sio
    activeContour_filename = 'C:\\Users\\remus\\OneDrive\\Documents\\Study\\Researches\\Projects\\cardiac\\Matlab_utils\\active_contour\\active_contour_fixed_para.mat'
    activeContour_data = sio.loadmat(activeContour_filename, struct_as_record=False, squeeze_me = True)['outputData']
    
    # Append active contour results
    # print('append AC')
    for activeContour_datum in activeContour_data:
        for datum in dataFull:
            datumSET_patient_name = datum['dataFilename'].split('Data_Jerry/')[-1].split('/mat_processed')[0]
            datum_SET_name, datum_patient_name = datumSET_patient_name.split('\\')
            datum_slice_name = datum['dataFilename'].split('processed_Jerry/')[-1].split('_processed')[0]
            if datum_SET_name in activeContour_datum.datafilename and datum_patient_name in activeContour_datum.datafilename and datum_slice_name in activeContour_datum.datafilename:
                datum['activeContourXs'] = activeContour_datum.activeContourResult
                # print(getSliceName(datum['dataFilename']))
                # print('Append!')
                break
...
for expIdx, config in enumerate(configs):
    ...
    train_indices, test_indices = move_patient_data_fromTrain_toTest(train_indices, test_indices, ['SET01\\CT11','SET02\\CT28'])
    ...
    # for datum in dataTr + dataTe:
    #     # datum_patient_slice_name = datum['dataFilename'].split('Jerry/')[-1].split('/mat_processed')[0]
    #     datum_patient_slice_name = datum['dataFilename'].split('Data_Jerry/')[-1].split('/mat_processed')[0]
    #     datum_patient_name, datum_slice_name = datum_patient_slice_name.split('\\')
    #     for activeContour_datum in activeContour_data:
    #         if datum_patient_name in activeContour_datum.datafilename and datum_slice_name in activeContour_datum.datafilename:
    #             datum['activeContourXs'] = activeContour_datum.activeContourResult
    
    # Append active contour results
    # for activeContour_datum in activeContour_data:
    #     for datum in dataFullRaw:
    #         if getSliceName(activeContour_datum.datafilename) == getSliceName(datum['dataFilename']):
    #             datum['activeContourResult'] = activeContour_datum.activeContourResult.reshape(1,-1)
    #             datum['activeContourResultFullRes'] = activeContour_datum.activeContourResultFullRes.reshape(1,-1)
    
    # activeContourPath  = './data/active_contour_ISBI_first_submission/'
    # import scipy.io as sio
    ...
    #     activeContourSlice = str(activeContourData['sliceName'][0])
...
# from utils.TOS3DPlotInterpFunc_ISBI_final import TOS3DPlotInterp
# dataTe_patientNames = np.unique([getPatientName(datum['dataFilename']) for datum in dataTe])
# dataTe_to_show_3D = [datum for datum in dataTe if getPatientName(datum['dataFilename']) == dataTe_patientNames[1]]
...
# vmax = None
# TOS3DPlotInterp(dataTe_to_show_3D, tos_key = 'TOS18_Jerry', title = dataTe_patientNames[1] + 'GT', restoreOriSlices = restoreOriSlices, vmax = vmax)
# TOS3DPlotInterp(dataTe_to_show_3D, tos_key = ACType, title = dataTe_patientNames[1] + 'active contour', restoreOriSlices = restoreOriSlices, vmax = vmax)
...
# TOS3DPlotInterp(dataTe_to_show_3D, tos_key = 'pred_pred', title = dataTe_patientNames[1] + 'joint reg', restoreOriSlices = restoreOriSlices, vmax = vmax)
        
#%% Rebuild 3D TOS Map
data_SET01_CT01 = [datum for datum in dataTe if 'SET01\\CT11' in datum['dataFilename']]
data_SET02_EC03 = [datum for datum in dataTe if 'SET02\\CT28' in datum['dataFilename']]
...
for sliceData in data_SET01_CT01:
    sliceData['TOSInterpolatedNew'] = interpTOS(sliceData['TOS18_Jerry'], sum(sliceData['AnalysisFv'].layerid == 1), len(np.unique(sliceData['AnalysisFv'].layerid)))
...
# data_SET01_CT01_show = [data_SET01_CT01[idx] for idx in range(len(data_SET01_CT01)) if idx not in [6]]
data_SET01_CT01_show = [data_SET01_CT01[idx] for idx in [0,1,2,4]]
# TOS3DPlotInterp(data_SET01_CT01_show, toshow = 'TOSInterpolatedNew', interpMethod = interpMethod, azim = 75, vmin=None, vmax = None)# GT
TOS3DPlotInterp(data_SET01_CT01_show, toshow = 'TOSPredInterpolated', interpMethod = interpMethod, azim = 75, vmin=None, vmax = None, addSectors = 0)# Pred
...
data_SET02_EC03_show = [data_SET02_EC03[idx] for idx in [0,1,3,5,6,7]]
# TOS3DPlotInterp(data_SET02_EC03_show, toshow = 'TOSInterpolatedNew', interpolate = True, vmin = None, vmax = 130, interpMethod = interpMethod, azim = -30)
TOS3DPlotInterp(data_SET02_EC03_show, toshow = 'TOSPredInterpolated', interpolate = True, vmin = None, vmax = 130, interpMethod = interpMethod, azim = -30)
