...
def getSliceName(filename):
    ...
    filename = filename.replace('\\', '/')         # Unify path format
...
config['data']['inputType'] = 'strainMatFullResolution'
config['data']['outputType'] = 'TOSInterpolatedMid'
...
if loadExtractedData:
    ...
    dataFilename = '../../../../../Dataset/dataFull-201-2020-11-04.npy'
    ...
    dataFull  = [datum for datum in dataFull if 'OLD' not in datum['dataFilename']]
...
# Take mid layer
if config['data']['outputType'] == 'TOSInterpolatedMid':
    for datum in dataFull:
...
for expIdx, config in enumerate(configs):
    ...
    net_config['paras']['n_sectors'] = inputDataTr.shape[-2]
    ...
    if showtest:
        ...
        def visTOsS(TOSs, strainMat = None, legends = None, title = None, axe = None, linewidth = 2, colors = ['k', 'b', 'orange', 'green'], ignore_zero_padding = True):
            ...
            if strainMat is not None:
                ...
                axe.pcolor(strainMat[:, :len(nonzero_col_indicator) - np.argmax(nonzero_col_indicator[::-1])], cmap='jet', vmin = -0.2, vmax = 0.2)
            for idx, TOS in enumerate(TOSs):
                TOSGrid = np.flip((TOS / 17).flatten() - 0.5)
                # TOSGrid = (TOS / 17).flatten() - 0.5
                line, = axe.plot(TOSGrid,np.arange(len(TOS.flatten()))+0.5, linewidth = linewidth, color = colors[idx])
        ...
        axs = axs.flatten()
        for idx, datum in enumerate(dataTr[:12]):
            visTOsS([datum[config['data']['outputType']],datum['pred']], datum[config['data']['inputType']], ['GT', 'Pred'], axe = axs[idx], title = getSliceName(datum['dataFilename']))
        fig.suptitle('Training - Original')
        
        fig, axs = plt.subplots(3,4)
        axs = axs.flatten()
        for idx, datum in enumerate(dataTe[:12]):
            visTOsS([datum[config['data']['outputType']],datum['pred']], datum[config['data']['inputType']], ['GT', 'Pred'], axe = axs[idx], title = getSliceName(datum['dataFilename']))
        fig.suptitle('Test')
